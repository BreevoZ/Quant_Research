...
`h5_base` 默认 `/home/sharedriver1/public/Lob_local`（在 main.cpp 中，非 CLI 参数）。
...
| `--parquet-out-dir <path>` | Parquet 分片根目录（默认 `/home/sharedriver1/public/Level2_MX`） |
...
`--output parquet` 写 **二进制分片** 到 `{parquet-out-dir}/.staging/{YYYYMMDD}/{symbol}.bin`，再用 `merge_3s_parquet.py` 合并为 `{YYYYMMDD}_tick.parquet`。

> ⚠️ **`merge_3s_parquet.py` 不在本项目里**（全盘也未找到）。要走 parquet 路线需先补回这个合并脚本；
> 在此之前可用 `scripts/compare_quick.py` 直接对 `.bin` 分片做校验，不必先合并。
> （`auction` 项目的 `run.sh` 有同类问题：引用了不存在的 `merge_action_parquet.py`。）
...
| `CMakeLists.txt` | 备用（`add_executable(lob_processor ...)`） | 三者产出同一个 `build/lob_processor` |
