# Historical notes · 原因子项目历史记录（中文）

This is a preserved historical document. Directory examples below use the former layout; see [Migration](../docs/MIGRATION.md) for current paths.

本文保留拆分前的历史说明；示例路径为旧布局，新位置见迁移文档。

# factor — 早期因子探索与评估框架验证

> ⚠️ **本项目已被 [`l3_factor`](../ashare/l3_factor/) 取代，不再维护。**
...
数据源：`<crypto_data>/{year}/{mm}/{date}.csv`，`crypto_data` 是 `qr.toml` 里的键，
默认 `data/massive/Crypto_MIN`，可用 `QR_CRYPTO_DATA` 覆盖。
