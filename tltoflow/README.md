...
```bash
# 最简: 单日
./tl_to_flow_csv -d 20260611

# 生产实际用法
./tl_to_flow_csv -d 20260611 \
    --tl-root      /home/TLData \
    --output-root  /home/sharedriver1/public/Lob_local/Flow_TL \
    --sort-key     appseq \
    --sort-tmp-dir /tmp \
    --missing-policy fail \
...
默认路径:输入 `/home/TLData/<DATE>`,输出 `/home/sharedriver1/public/Flow_TL/<DATE>.csv`。
...
|---|---|---|
| `-d, --date DATE` | — | 交易日期(必需) |
| `--input-dir DIR` / `--tl-root DIR` | `/home/TLData` | 输入目录 / 根目录 |
| `--output-file FILE` / `--output-root DIR` | `/home/sharedriver1/public/Flow_TL` | 输出文件 / 根目录 |
...
```bash
g++ -O3 -std=c++17 -march=native tl_to_flow_csv.cpp -o tl_to_flow_csv
```

无第三方依赖。`CMakeLists.txt` 亦可用。
