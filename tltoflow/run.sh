...
BIN=${BIN:-"$SCRIPT_DIR/tl_to_flow_csv"}
TL_ROOT=${TL_ROOT:-/home/TLData}
OUTPUT_ROOT=${OUTPUT_ROOT:-/SSDDATA2/sharedriver1/public/Lob_local/Flow_TL}
SORT_TMP_DIR=${SORT_TMP_DIR:-/SSDDATA2/tmp}
...
if [[ ! -x $BIN ]]; then
  ...
  echo "build it first, e.g. g++ -O3 -std=c++17 -Wall -Wextra -o $BIN $SCRIPT_DIR/tl_to_flow_csv.cpp" >&2
  exit 1
fi
