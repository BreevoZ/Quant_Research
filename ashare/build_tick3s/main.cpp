...
void printUsage(const char* prog_name) {
    ...
    std::cout << "  --h5-dir <path>          逐笔 LOB 的 H5 输入根目录\n";
    std::cout << "                           (默认: " << qr::get("lob_h5") << ")\n";
    std::cout << "  --level2-dir <path>      Level2 快照输入根目录\n";
    std::cout << "                           (默认: " << qr::get("lob_level2") << ")\n";
    std::cout << "  --csv-dir <path>         3 秒切片 CSV 输出根目录\n";
    std::cout << "                           (默认: " << qr::get("lob_3s") << ")\n";
    std::cout << "  --parquet-out-dir <path>  Parquet 输出根目录\n";
    std::cout << "                           (默认: " << qr::get("tick3s_parquet") << ")\n";
    std::cout << "                           分片写入 {path}/.staging/{YYYYMMDD}/{symbol}.bin\n";
    std::cout << "                           需手动执行 merge_3s_parquet.py 合并为 {YYYYMMDD}_tick.parquet\n\n";
    std::cout << "\n以上四个目录的默认值来自 qr.toml / QR_LOB_H5 等环境变量, 见 qr/qr_paths.h。\n\n";
    std::cout << "参数:\n";
...
LobDataProcessor::ProcessResult processDate(
    ...
    const std::string& h5_base_path,
    const std::string& level2_base_path,
    const std::string& csv_base_path,
...
int main(int argc, char* argv[]) {
    try {
        // ClickHouse 配置。口令不进源码, 一律从环境变量读:
        //   QR_CH_HOST / QR_CH_PORT / QR_CH_USER / QR_CH_PASSWORD / QR_CH_DATABASE
        ClickHouseConfig ch_config;
        ch_config.host = qr::env_or("QR_CH_HOST", "127.0.0.1");
        // clickhouse-cpp 用 TCP 端口(9000), 不是 HTTP 端口(8123)
        ch_config.port = std::stoi(qr::env_or("QR_CH_PORT", "9000"));
        ch_config.username = qr::env_or("QR_CH_USER", "default");
        ch_config.password = qr::env_or("QR_CH_PASSWORD");
        ch_config.database = qr::env_or("QR_CH_DATABASE", "Tick_TL");
        ...
        // 四个目录的默认值来自 qr.toml / QR_* 环境变量, 命令行可再覆盖
        std::string h5_base_path = qr::get("lob_h5");
        std::string level2_base_path = qr::get("lob_level2");
        std::string csv_base_path = qr::get("lob_3s");
        std::string parquet_out_dir = qr::get("tick3s_parquet");
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--market" || arg == "-m") {
                ...
                if (i + 1 < argc) {
                    h5_base_path = argv[i + 1];
                    ++i;
                } else {
                    std::cerr << "Error: --h5-dir requires a value" << std::endl;
                    return 1;
                }
            } else if (arg == "--level2-dir") {
                if (i + 1 < argc) {
                    level2_base_path = argv[i + 1];
                    ++i;
                } else {
                    std::cerr << "Error: --level2-dir requires a value" << std::endl;
                    return 1;
                }
            } else if (arg == "--csv-dir") {
                if (i + 1 < argc) {
                    csv_base_path = argv[i + 1];
                    ++i;
                } else {
                    std::cerr << "Error: --csv-dir requires a value" << std::endl;
                    return 1;
                }
