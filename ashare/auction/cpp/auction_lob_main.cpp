 ...
 * 输出目录由 qr/qr_paths.h 解析(QR_LOB_ROOT / qr.toml), 其下按 <DATE>/ 分目录
 *
 * 用法:
...
// ============================================================
// 默认路径由 qr/qr_paths.h 解析:环境变量 QR_TL_ZIP / QR_LOB_ROOT → qr.toml → 内置默认。
// 用函数而非全局常量,避免静态初始化期抛异常时拿不到错误信息。
const std::string& DEFAULT_TL_BASE_PATH() {
    static const std::string v = qr::get("tl_zip");
    return v;
}
const std::string& DEFAULT_OUTPUT_PATH() {
    static const std::string v = qr::get("lob_root");
    return v;
}
