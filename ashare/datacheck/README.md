...
Reference limit-order-book reconstruction and dual-source validation. Rule changes must be checked against the simulator and auction copies.
...
`cd ashare/datacheck` unless they explicitly start from the repository root.

## 中文技术说明

下文保留原项目技术细节；项目内相对命令以 `ashare/datacheck` 为工作目录，仓库根命令另行标明。
...
优先使用系统 HDF5；仓库 `third_party/hdf5/` 只是历史回退，部分 Linux 库软链接缺少目标，不能替代系统安装。详见公共环境说明。

---

## 数据源

`datacheck` 的所有路径都是必填参数，没有内置默认值。下面用 `qr.toml` 里的键名表示位置，
本机实际路径用 `python3 -c "from qr import paths; print(paths.get('tl_zip'))"` 查。

| 类型 | 路径示例 | 格式 |
|---|---|---|
| Flow | `<flow_root>/{date}/` | 目录，每个时间片一个 CSV，沪深均有 |
| TL SH | `<tl_zip>/{date}/mdl_4_24_0.csv` | 单文件，含 BizIndex / Type A/D/T |
| TL SZ 委托 | `<tl_zip>/{date}/mdl_6_33_0.csv` | 深圳委托，含 ApplSeqNum/Side/OrdType |
| TL SZ 成交 | `<tl_zip>/{date}/mdl_6_36_0.csv` | 深圳成交/撤单，含 BidApplSeqNum/OfferApplSeqNum/ExecType |
| TL LOB H5 | `<lob_root>/{date}/{sh\|sz}{sym}.h5` | compound-type HDF5，含 order / trade_1500 |
...
| 位置 | 关系 | 状态 |
|---|---|---|
| `datacheck/lob_builder.cpp` | **基准** | — |
| `vorder_sim/lob_builder.cpp` | **拷贝 + 附加扩展** | 相对基准 +112 行 |
| `auction/cpp/auction_lob_builder.cpp` | **独立实现**，靠移植保持一致 | 源码内 6 处标注「移植自 datacheck_final」 |
...
上面这句「目前是一致的」已经固化成基线，由脚本守着：

```bash
python scripts/check_lob_drift.py            # 有基线之外的新差异就退出码 1
python scripts/check_lob_drift.py --update   # 三处同步复核过后刷新基线
```

它比对基准与 `vorder_sim` 那份的逐行差异（忽略行尾空白），再数一遍 `auction` 里
「移植自」标注的条数，两项都对上才算没漂移。基线在
`scripts/lob_builder_drift.baseline`，改动任意一份都会被它挡下。

不需要外部数据，`cmake -B build -S . && ctest --test-dir build` 会连带跑它。

---

## datacheck 用法

下面的例子先用 `qr/qr_paths.sh` 把路径读进 shell 变量，这样换机器只改 `qr.toml`，
命令本身不用动：

```bash
source qr/qr_paths.sh          # 在仓库根执行
TL=$(qr_path tl_zip)
FLOW=$(qr_path flow_root)
LOB=$(qr_path lob_root)
OUT=${TMPDIR:-/tmp}/dc_out     # 对比产物，放哪儿都行
```
...
./build/datacheck \
  --all --sym-prefix 6 \
  --date ${DATE} --market sh \
  --primary-type tl --primary-path ${TL}/${DATE}/mdl_4_24_0.csv \
  --compare-type flow --compare-path ${FLOW}/${DATE} \
  --output-dir ${OUT}/${DATE} \
  --gen-lob --output-format h5 \
  --lob-dir ${OUT}/${DATE}/lob_h5 \
  --workers 40
```
...
```bash
./build/lob_compare \
  --dir-a ${OUT}/${DATE}/lob_h5 \
  --dir-b ${LOB}/${DATE} \
  --file-prefix-b sh \
  --sysid-mod-b 1000000000000 \
  --label-a dc --label-b tl \
  --date ${DATE} \
  --output ${OUT}/${DATE}/cmp_dc_tl_lob.md \
  --workers 40
```
...
./build/datacheck \
  ...
  --primary-type tl --primary-path ${TL}/${DATE}/mdl_6_33_0.csv \
  --primary-path2 ${TL}/${DATE}/mdl_6_36_0.csv \
  --compare-type flow --compare-path ${FLOW}/${DATE} \
  --output-dir ${OUT}/${DATE} \
  --gen-lob --output-format h5 \
  --workers 40
...
```bash
./build/lob_compare \
  --dir-a ${OUT}/${DATE}/lob_sz \
  --dir-b ${LOB}/${DATE} \
  --file-prefix-b sz \
  --sysid-mod-b 1000000000000 \
  --label-a dc --label-b tl \
  --date ${DATE} \
  --output ${OUT}/${DATE}/cmp_dc_tl_lob_sz.md \
  --workers 40
```
...
./build/datacheck \
  --all --sym-prefix 6 \
  --date ${DATE} --market sh \
  --primary-type flow --primary-path ${FLOW}/${DATE} \
  --output-dir ${OUT}/${DATE} \
  --gen-lob --output-format h5 \
  --lob-dir ${OUT}/${DATE}/lob_flow_h5 \
  --workers 40
```
...
./build/datacheck \
  --all --date 20260611 \
  --primary-type tl --primary-path ${TL}/20260611/mdl_4_24_0.csv \
  --compare-type flow --compare-path ${FLOW}/20260611 \
  --output-dir ${OUT}/20260611 \
  --workers 40
```
...
```bash
./build/lob_compare \
  --dir-a ${OUT}/20260611/lob_h5 \
  --dir-b ${LOB}/20260611 \
  --file-prefix-b sh \
  --sysid-mod-b 1000000000000 \
  --label-a dc --label-b tl \
  --date 20260611 \
  --output ${OUT}/20260611/cmp_dc_tl_lob.md \
  --workers 40
```
