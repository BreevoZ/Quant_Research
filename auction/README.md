...
```
/home/TLData/<date>/mdl_*.csv[.zip]
        │  (直接吃原始通联 mdl 行情, 不经 Flow 转换)
        ▼
  auction_lob_main  ──►  {sh|sz}{symbol}_action.bin   每股一个分片
        │                 (紧凑列主序二进制)
        ▼
  merge_action_parquet.py  ──►  <date>_auction.parquet   当日全市场
```
...
依赖:CMake、Boost、HDF5(仓库根 `third_party/hdf5/` 已自包含,不依赖原编译环境路径)。
...
> `third_party/hdf5/lib` 下的动态库已改为完全自包含,不再引用原编译环境中某个不相关项目的路径。
...
```bash
cd cpp
./auction_lob_main -d 20251231                   # 单日全市场
./auction_lob_main -d 20251201 -e 20251231       # 日期区间
./auction_lob_main -d 20251231 -s 600638 -m SH   # 单只股票
./auction_lob_main -d 20251231 -m SZ             # 只处理深圳
./auction_lob_main -d 20251231 -f /path/TLData -o /path/output
```
...
1. **`run.sh` 引用了不存在的文件**(⚠️ 当前生产流程第 3 步跑不通)
   脚本调用 `$project_dir/merge_action_parquet.py`,但该文件在本项目中不存在
   (`auction/` 下目前只有 `cpp/` 一层)。跑批前需补回这个 merge 脚本,或改用其他方式合并分片。
